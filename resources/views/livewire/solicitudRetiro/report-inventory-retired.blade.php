<div>
    ...
    <body>
        <div class="header">
            <h2>Solicitud de Anulación de Equipos</h2>
        </div>
        <div id="footer">
            <p class="textFooter">
    ...
    <tbody>
        ...
        @foreach ($movements as $movement)
            <tr>
                ...
                @if ($movement->message_request)
                    <td><img style=" max-width: 10px;
                    max-height: 8px;
                    margin-right: 8px;"
                            src="{{ asset('img/x.png') }}"></td>
                @else
                    <td><img style=" max-width: 10px;
                                max-height: 10px;
                                margin-right: 10px;"
                            src="{{ asset('img/check.png') }}"></td>
                @endif
            </tr>
            @if ($movement->message_request)
                <tr style=" background-color: #ddd7d750">
                    <td colspan="4">
                        <label style="font-weight: bold">Motivo de Retiro:</label>
                        {{ $movement->message }}
                    </td>
                    <td colspan="5">
                        <label style="color: red; font-weight: bold">Solicitud Rechazada:</label>
                        {{ $movement->message_request }}
                    </td>
                </tr>
            @else
                <tr style=" background-color: #ddd7d750">
...
<div>
    ...
    <label>Las solicitudes rechazadas vuelven a almacen.</label>    
    <br>

<div>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h2 class="py-1 text-lg font-bold leading-tight tracking-tight text-dark-900 md:text-2xl dark:text-dark">
            Lista de Solicitudes de Crear o Incrementar Equipos
        </h2>
     
        {{-- Inicio - Lista de Transferencia de Movimientos --}}
        <x-table>
                    ...
                    <thead class="bg-gray-50 sticky top-0">
                        <tr>
                            ...
                            <th scope="col"
                                ...
                                @if ($sort == 'updated_at')
                                    @if ($direction == 'asc')
                                        <i class="fas fa-sort-alpha-up-alt ">&nbsp;</i>Fecha de Revision
                                    @else
                                        <i class="fas fa-sort-alpha-down-alt ">&nbsp;</i>Fecha de Revision
                                    @endif
                                @else
                                    <i class= "fas fa-sort ">&nbsp;</i>Fecha de Revision
                                @endif
                            </th>
                            <th scope="col"
                            ...
                            @php
                                $can = Gate::allows('admin.request.see') || Gate::allows('admin.request.pdf');
                            @endphp
                            @if ($can)
                                <th scope="col"
                                    class="px-1 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($movements as $movement)
                            <tr wire:key="movement-{{ $movement->receipt_number }}">
                                ...
                                @else
                                    @if ($movement->state_create == 0)
                                        <td class="px-1 text-center"
                                            style="padding-top: 0.150rem;   padding-bottom: 0.150rem;">
                                            <span
                                                class="inline-flex items-center bg-blue-400 text-white text-xs font-medium px-2.5 py-0.5 rounded-full ">
                                                <span class="w-2 h-2 me-1 bg-blue-600 rounded-full"></span>
                                                Pendiente | Admin.
                                            </span>
                                        </td>
                                    @else
                                        @if ($movement->state_create == 2)
                                            <td class="px-1 text-center"
                                                style="padding-top: 0.150rem;   padding-bottom: 0.150rem;">
                                                <span
                                    ...
                                    @endif
                                @endif
                                <td class=" pt-1 flex " style="padding-top: 0.150rem;   padding-bottom: 0.150rem;">
                                    <div class="">
                                        @can('admin.request.see')
                                            @livewire('solicitudCrear.view-inventory-transfer', ['receipt_number' => $movement->receipt_number], key($movement->receipt_number))
                                        @endcan
                                    </div>
                                    @can('admin.request.pdf')
                                        @if ($movement->state_create == 1)
                                            <div class="pl-1">
                                                <form method="POST" action="{{ route('downloadResquestCreate') }}">
                                                    @csrf
                                                    <div>
                                                        <button type="submit" value="{{ $movement->receipt_number }}"
                                                            name="request_receipt_number"

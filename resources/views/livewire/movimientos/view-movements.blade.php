<div>
                    ...
                    <div class="bg-white  rounded-lg ">
                        <div class="flex pt-3">
                            <h3 class="font-bold text-base align-middle m-0 px-3">
                                Nro {{ $receipt_number }}: Seleccione Cantidad Herramienta
                            </h3>
                                ...
                                <tbody class="divide-y divide-gray-200 border-y">
                                    @foreach ($inventories as $inventory)
                                        <tr class="bg-stone-100" wire:key="inventory-{{ $inventory->id_inventory }}">
                                            @can('admin.movement.edit')
                                                @if ($inventory->missing_amount == $inventory->return_amount && $stateButton != 3)
                                                ...
                                                @else
                                                    ...
                                                    @else
                                                        <td class="px-1 pt-2 text-center">
                                                            <i
                                                                class="fa-solid fa-square-check focus:ring-green-500 fa-lg "></i>
                                                        </td>
                                                    @endif
                                                @endif
                                            @endcan
    ...
    $inventory->missing_amount -
        $inventory->return_amount +
        App\Models\MovementHistory::where('id_movements', $inventory->id_movements)->orderBy('id', 'desc')->value('return_amount');
    $i++)
                                                                <option value="{{ $i }}">
                                                                    {{ $i }}
                                                                </option>
